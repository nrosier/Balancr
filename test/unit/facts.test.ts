...
describe('persistFacts', () => {
  it('is idempotent: the same input twice leaves the same data', () => {
    ...
    expect(persistFacts(ctx.db, TENANT_ID, facts, ['2026-01'])).toEqual({ written: 2, removed: 0 })
    const first = rows()
  ...
  it('updates every column in place rather than inserting a second row', () => {
    persistFacts(ctx.db, TENANT_ID, [fact('2026-01', 'food')], ['2026-01'])
    persistFacts(ctx.db, TENANT_ID,
      [
        fact('2026-01', 'food', {
          spentCents: 55_000,
  ...
  it('keeps a null baseline null instead of storing a zero norm', () => {
    ...
    persistFacts(ctx.db, TENANT_ID, [fact('2026-01', 'new')], ['2026-01'])
  ...
  it('removes a category that no longer appears in a recomputed month', () => {
    ...
    persistFacts(ctx.db, TENANT_ID, [fact('2026-01', 'food'), fact('2026-01', 'gone')], ['2026-01'])
    expect(persistFacts(ctx.db, TENANT_ID, [fact('2026-01', 'food')], ['2026-01'])).toEqual({
      written: 1,
      removed: 1,
    })
  ...
  it('clears a month that legitimately ends up with no categories', () => {
    ...
    persistFacts(ctx.db, TENANT_ID, [fact('2026-01', 'food')], ['2026-01'])
    expect(persistFacts(ctx.db, TENANT_ID, [], ['2026-01'])).toEqual({ written: 0, removed: 1 })
    expect(rows()).toEqual([])
  })

  it('leaves months outside the recomputed window alone', () => {
    persistFacts(ctx.db, TENANT_ID, [fact('2025-12', 'food')], ['2025-12'])
    persistFacts(ctx.db, TENANT_ID, [fact('2026-01', 'food')], ['2026-01'])
    expect(rows()).toHaveLength(2)

    persistFacts(ctx.db, TENANT_ID, [], ['2026-01'])
  ...
  it('writes more rows than one statement holds', () => {
    ...
    expect(persistFacts(ctx.db, TENANT_ID, many, ['2026-01'])).toEqual({ written: 450, removed: 0 })
...
describe('syncCategoryMeta', () => {
  it('records what the user told us and never overwrites it', () => {
    ...
    syncCategoryMeta(ctx.db, TENANT_ID, [fact('2026-01', 'c1', { categoryName: 'Boodschappen' })])
    ctx.db
      .update(categoryMeta)
    ...
    syncCategoryMeta(ctx.db, TENANT_ID, [fact('2026-02', 'c1', { categoryName: 'Groceries' })])
  ...
  it('takes the name from the latest month, whatever order the facts arrive in', () => {
    syncCategoryMeta(ctx.db, TENANT_ID, [
  ...
  it('marks an income category as such on first sight', () => {
    syncCategoryMeta(ctx.db, TENANT_ID, [fact('2026-01', 'salary', { isIncome: true })])
    expect(
...
describe('loadFrequencies', () => {
  it('round-trips what aggregateSpend asks for, and omits what it has not been told', () => {
    ...
    syncCategoryMeta(ctx.db, TENANT_ID, [fact('2026-01', 'insurance'), fact('2026-01', 'food')])
    ctx.db
      .update(categoryMeta)
    ...
    const frequencies = loadFrequencies(ctx.db, TENANT_ID)
...
describe('loadFacts', () => {
  it('reads back what was written, baseline and all', () => {
    ...
    syncCategoryMeta(ctx.db, TENANT_ID, [written])
    persistFacts(ctx.db, TENANT_ID, [written], ['2026-03'])

    expect(loadFacts(ctx.db, TENANT_ID, '2026-03')).toEqual([written])
  })
  ...
  it('takes the name, income flag and hidden flag from the meta row, not the fact row', () => {
    ...
    syncCategoryMeta(ctx.db, TENANT_ID, [march])
    persistFacts(ctx.db, TENANT_ID, [march], ['2026-03'])
    ctx.db
      .update(categoryMeta)
    ...
    const loaded = loadFacts(ctx.db, TENANT_ID, '2026-03')[0]
  ...
  it('is ordered by category id, so a payload labels the same category the same way', () => {
    ...
    syncCategoryMeta(ctx.db, TENANT_ID, facts)
    persistFacts(ctx.db, TENANT_ID, facts, ['2026-03'])
    expect(loadFacts(ctx.db, TENANT_ID, '2026-03').map((f) => f.categoryId)).toEqual(['food', 'gas', 'rent'])
  })

  it('reads one month only', () => {
    const facts = [fact('2026-02', 'food'), fact('2026-03', 'food')]
    syncCategoryMeta(ctx.db, TENANT_ID, facts)
    persistFacts(ctx.db, TENANT_ID, facts, ['2026-02', '2026-03'])
    expect(loadFacts(ctx.db, TENANT_ID, '2026-03').map((f) => f.month)).toEqual(['2026-03'])
  })

  it('is empty for a month that was never computed', () => {
    expect(loadFacts(ctx.db, TENANT_ID, '2026-03')).toEqual([])
  })
  ...
  it('skips a fact whose category has no meta row', () => {
    ...
    persistFacts(ctx.db, TENANT_ID, [fact('2026-03', 'orphan')], ['2026-03'])
    expect(loadFacts(ctx.db, TENANT_ID, '2026-03')).toEqual([])
  })
  ...
  it('keeps a null delta on a baseline that has one', () => {
    ...
    syncCategoryMeta(ctx.db, TENANT_ID, [written])
    persistFacts(ctx.db, TENANT_ID, [written], ['2026-03'])
    expect(loadFacts(ctx.db, TENANT_ID, '2026-03')[0]?.baseline).toEqual(written.baseline)
  })
  ...
  it('round-trips the day-of-month curve (#311)', () => {
    ...
    syncCategoryMeta(ctx.db, TENANT_ID, [written])
    persistFacts(ctx.db, TENANT_ID, [written], ['2026-03'])
    expect(loadFacts(ctx.db, TENANT_ID, '2026-03')[0]?.dayCurve).toEqual(written.dayCurve)
  })
...
describe('loadCategoryMeta', () => {
  it('keys every stored row by its category id', () => {
    syncCategoryMeta(ctx.db, TENANT_ID, [fact('2026-03', 'food'), fact('2026-03', 'rent')])
    const meta = loadCategoryMeta(ctx.db, TENANT_ID)
  ...
  it('carries the fields the redaction boundary reads', () => {
    ...
    syncCategoryMeta(ctx.db, TENANT_ID, [fact('2026-03', 'therapy')])
    ctx.db
      .update(categoryMeta)
      .set({
    ...
    const row = loadCategoryMeta(ctx.db, TENANT_ID).get('therapy')
    expect(row).toMatchObject({
      userDescription: 'Weekly sessions',
      coicopCode: '06.2.2',
      nature: 'fixed',
  ...
  it('is empty before the first sync', () => {
    expect(loadCategoryMeta(ctx.db, TENANT_ID).size).toBe(0)
  })
})
...
describe('loadCategoryTrends', () => {
  ...
  it('returns the window oldest first, ending at the month asked for', () => {
    seed()
    expect(loadCategoryTrends(ctx.db, TENANT_ID, '2026-03', 3).months).toEqual([
      '2026-01',
      '2026-02',
      '2026-03',
    ])
  ...
  it('aligns every series to that window', () => {
    seed()
    const trends = loadCategoryTrends(ctx.db, TENANT_ID, '2026-03', 3)
  ...
  it('reaches back past the stored history without inventing months', () => {
    seed()
    const trends = loadCategoryTrends(ctx.db, TENANT_ID, '2026-03', 5)
  ...
  it('ignores months after the one asked for', () => {
    ...
    seed()
    const trends = loadCategoryTrends(ctx.db, TENANT_ID, '2026-02', 2)
