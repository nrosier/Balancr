...
describe('per-tenant token isolation (#371)', () => {
  it("caches tokens per tenant, so one tenant's fetch never authenticates as another", async () => {
    ...
    await fetchPortfolioDetails(dbA, getSoleTenantId(dbA))
    await fetchPortfolioDetails(dbB, getSoleTenantId(dbB))
    await fetchPortfolioDetails(dbA, getSoleTenantId(dbA)) // cached under tenant-a's own entry, not tenant-b's
  ...
  it('resets only the named tenant, leaving the other tenant cached', async () => {
    ...
    await fetchPortfolioDetails(dbA, getSoleTenantId(dbA)) // re-authenticates
    await fetchPortfolioDetails(dbB, getSoleTenantId(dbB)) // still cached — untouched by tenant-a2's reset
...
describe('authentication', () => {
  ...
  it('gives up after a second 401 instead of looping', async () => {
    ...
    await expect(fetchPortfolioDetails(db, tenantId)).rejects.toThrow(GhostfolioError)
...
describe('errors carry the offending path', () => {
  it('reports an HTTP failure with its status', async () => {
    ...
    const error = await fetchPortfolioDetails(db, tenantId).catch((e: unknown) => e)
  ...
  it('reports a shape change without a status, which is how the probe tells them apart', async () => {
    ...
    const error = (await fetchPortfolioDetails(db, tenantId).catch((e: unknown) => e)) as GhostfolioError
  ...
  it('treats a non-JSON body as a failure of that path', async () => {
    ...
    await expect(fetchPortfolioDetails(db, tenantId)).rejects.toThrow(/did not return JSON/)
  })
})
...
describe('the twice-versioned performance endpoint (#115)', () => {
  ...
  it('does not fall back on any status but 404', async () => {
    ...
    await expect(fetchPortfolioPerformance(db, tenantId)).rejects.toThrow(GhostfolioError)
    expect(performancePaths()).toEqual(['/api/v2/portfolio/performance'])
  })
  ...
  it('reports the older path when neither version answers', async () => {
    ...
    const error = await fetchPortfolioPerformance(db, tenantId).catch((e: unknown) => e)
...
describe('probe', () => {
  ...
  it('a shape mismatch outranks an outage in the overall status', async () => {
    ...
    expect((await probeGhostfolio(db, tenantId)).status).toBe('shape-mismatch')
  })
  ...
  it('starts from a fresh token so a stale JWT is diagnosed here', async () => {
    ...
    await probeGhostfolio(db, tenantId)
...
describe('the evidence a Ghostfolio account carries about itself', () => {
  it('reads the activity count and the converted balance when the instance sends them', async () => {
    ...
    const [account] = (await fetchAccounts(db, tenantId)).accounts
