...
describe('persistSignals', () => {
  it('round-trips a signal, metrics included', () => {
    expect(persistSignals(ctx.db, tenantId, '2026-03', [signal()], clean)).toEqual({ signals: 1 })
    expect(loadSignals(ctx.db, tenantId, '2026-03')).toEqual([signal()])
  })
  ...
  it('keeps a household signal, whose subject is nothing', () => {
    ...
    persistSignals(ctx.db, tenantId, '2026-03', [household], clean)
    expect(loadSignals(ctx.db, tenantId, '2026-03')).toEqual([household])
  })
  ...
  it('stores two stale accounts as two rows, not one', () => {
    ...
    expect(persistSignals(ctx.db, tenantId, '2026-03', rows, clean)).toEqual({ signals: 2 })
    expect(loadSignals(ctx.db, tenantId, '2026-03')).toHaveLength(2)
  })
  ...
  it('replaces the month wholesale on a re-run', () => {
    persistSignals(ctx.db, tenantId, '2026-03', [signal(), signal({ code: 'over_assigned', severity: 'warn' })], clean)
    persistSignals(ctx.db, tenantId, '2026-03', [signal()], clean)
    expect(loadSignals(ctx.db, tenantId, '2026-03').map((s) => s.code)).toEqual(['over_available'])
  })

  it('leaves other months alone', () => {
    persistSignals(ctx.db, tenantId, '2026-02', [signal()], clean)
    persistSignals(ctx.db, tenantId, '2026-03', [], clean)
    expect(loadSignals(ctx.db, tenantId, '2026-02')).toHaveLength(1)
    expect(loadSignals(ctx.db, tenantId, '2026-03')).toHaveLength(0)
  })

  it('clears a month that now has nothing to report', () => {
    persistSignals(ctx.db, tenantId, '2026-03', [signal()], clean)
    expect(persistSignals(ctx.db, tenantId, '2026-03', [], clean)).toEqual({ signals: 0 })
    expect(loadSignals(ctx.db, tenantId, '2026-03')).toEqual([])
  })
  ...
  it('writes the hygiene row even for a month with no signals', () => {
    ...
    persistSignals(ctx.db, tenantId, '2026-03', [], clean)
    expect(loadHygiene(ctx.db, tenantId, '2026-03')).toEqual(clean)
  })

  it('updates the score in place rather than accumulating rows', () => {
    persistSignals(ctx.db, tenantId, '2026-03', [], { scoreBp: 8_000, deductions: [{ reason: 'x', bp: 2_000 }] })
    persistSignals(ctx.db, tenantId, '2026-03', [], clean)
    expect(ctx.db.select().from(monthlyHygiene).all()).toHaveLength(1)
    expect(loadHygiene(ctx.db, tenantId, '2026-03')?.scoreBp).toBe(10_000)
  })
})
...
describe('loadSignals is defensive about what it reads', () => {
  it('is empty for a month that has never been judged', () => {
    expect(loadSignals(ctx.db, tenantId, '2026-03')).toEqual([])
    expect(loadHygiene(ctx.db, tenantId, '2026-03')).toBeNull()
  })
  ...
  it('keeps only the finite numbers in a metrics object', () => {
    ...
    ctx.db
      ...
      .where(eq(monthlySignals.month, '2026-03'))
      .run()
    expect(loadSignals(ctx.db, tenantId, '2026-03')[0]?.metrics).toEqual({ overspendCents: 8_000 })
  })
})

describe('loadSignalsForMonths (#352)', () => {
  it('keys each requested month to its own signals', () => {
    persistSignals(ctx.db, tenantId, '2026-01', [signal({ code: 'over_assigned', severity: 'warn' })], clean)
    persistSignals(ctx.db, tenantId, '2026-03', [signal()], clean)

    const byMonth = loadSignalsForMonths(ctx.db, tenantId, ['2026-01', '2026-02', '2026-03'])
  ...
  it('gives an empty array, not a missing entry, for a month with nothing stored', () => {
    ...
    const byMonth = loadSignalsForMonths(ctx.db, tenantId, ['2026-02', '2026-03'])
  ...
  it('is empty for an empty month list, without querying anything', () => {
    persistSignals(ctx.db, tenantId, '2026-03', [signal()], clean)
    expect(loadSignalsForMonths(ctx.db, tenantId, [])).toEqual(new Map())
  })
  ...
  it('drops a row whose code is no longer in the vocabulary, same as loadSignals', () => {
    ...
    expect(loadSignalsForMonths(ctx.db, tenantId, ['2026-03']).get('2026-03')).toEqual([])
  })
})
...
describe('loadHygiene is defensive too', () => {
  it('round-trips the deductions that explain the score', () => {
    ...
    persistSignals(ctx.db, tenantId, '2026-03', [], score)
    expect(loadHygiene(ctx.db, tenantId, '2026-03')).toEqual(score)
  })
  ...
  it('keeps the score when the breakdown is unreadable', () => {
    ...
    persistSignals(ctx.db, tenantId, '2026-03', [], { scoreBp: 6_500, deductions: [] })
    ctx.db
      .update(monthlyHygiene)
      .set({ deductionsJson: '{oops' })
      .where(eq(monthlyHygiene.month, '2026-03'))
      .run()
    expect(loadHygiene(ctx.db, tenantId, '2026-03')).toEqual({ scoreBp: 6_500, deductions: [] })
  })
  ...
  it('drops a deduction entry that is not one', () => {
    ...
    ctx.db
      ...
      .where(eq(monthlyHygiene.month, '2026-03'))
      .run()
    expect(loadHygiene(ctx.db, tenantId, '2026-03')?.deductions).toEqual([
      { reason: 'uncategorised', bp: 2_000 },
    ])
  })
})
...
describe('staleMonths (#162)', () => {
  it('is empty for no months', () => {
    expect(staleMonths(ctx.db, tenantId, [])).toEqual([])
  })

  it('ignores a month whose judged hash still matches its facts', () => {
    persistMonthTotals(ctx.db, tenantId, [totals('2026-03')], [], new Map([['2026-03', 'hash-a']]))
    persistSignals(ctx.db, tenantId, '2026-03', [], clean, 'hash-a')
    expect(staleMonths(ctx.db, tenantId, ['2026-03'])).toEqual([])
  })
  ...
  it('picks up a month whose facts moved since it was judged', () => {
    ...
    persistMonthTotals(ctx.db, tenantId, [totals('2026-03')], [], new Map([['2026-03', 'hash-b']]))
    expect(staleMonths(ctx.db, tenantId, ['2026-03'])).toEqual(['2026-03'])
  })
  ...
  it('leaves out a month with no stored fact hash at all', () => {
    ...
    persistMonthTotals(ctx.db, tenantId, [totals('2026-03')], [])
    expect(staleMonths(ctx.db, tenantId, ['2026-03'])).toEqual([])
  })
