...
const rows = () => loadAccountMap(ctx.db, TENANT_ID).sort((a, b) => a.externalId.localeCompare(b.externalId))
...
describe('syncAccountMap', () => {
  it('creates a row per sighting with the defaulted kind', () => {
    const result = syncAccountMap(ctx.db, TENANT_ID, [
  ...
  it('is idempotent: a second identical pass changes nothing', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Zichtrekening')])
    const first = rows()[0]

    const again = syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Zichtrekening')])
  ...
  it('separates the two sources: the same external id is two accounts', () => {
    ...
    const result = syncAccountMap(ctx.db, TENANT_ID, [actual('same', 'Actual side'), ghostfolio('same', 'GF side')])
    expect(result.created).toBe(2)
  })

  it('updates a name and nothing else', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Beleggingen', true)])
    ...
    const result = syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Beleggingsrekening', true)])
  ...
  it('refreshes off-budget on every sync without counting it as a rename', () => {
    ...
    // first sync (#353).
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Beleggingen', false)])
    expect(rows()[0]).toMatchObject({ offBudget: false })

    const result = syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Beleggingen', true)])
  ...
  it('never sets off-budget for a Ghostfolio sighting, which has no such concept', () => {
    syncAccountMap(ctx.db, TENANT_ID, [ghostfolio('g1', 'Bolero')])
    expect(rows()[0]).toMatchObject({ offBudget: false })
  })

  it('keeps a dedupe decision across a rename', () => {
    // The regression this whole module exists to prevent.
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Beleggingen', true), ghostfolio('g1', 'Bolero')])
    const [mirror, truth] = rows()
    setDedupeGroup(ctx.db, TENANT_ID, 'broker', [mirror!.id, truth!.id], truth!.id)

    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Beleggingen (oud)', true), ghostfolio('g1', 'Bolero NV')])
  ...
  it('reports an unseen account instead of deleting it', () => {
    ...
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Zichtrekening'), ghostfolio('g1', 'Bolero')])

    const result = syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Zichtrekening')])
...
describe('accountMapBySource', () => {
  it('keys each source by the id that source uses', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Zichtrekening'), ghostfolio('g1', 'Bolero')])
    const all = loadAccountMap(ctx.db, TENANT_ID)
...
describe('dedupeCandidates', () => {
  ...
  const asKind = (externalId: string, kind: 'cash' | 'investment' | 'savings'): string => {
    ...
    updateAccountMap(ctx.db, TENANT_ID, row!.id, { kind })
    return row!.id
  }
  ...
  it('offers nothing for a brokerage account and four unrelated cash accounts', () => {
    ...
    expect(dedupeCandidates(loadAccountMap(ctx.db, TENANT_ID))).toEqual([])
  })
  ...
  it('pairs a Ghostfolio cash mirror with its Actual twin, on name and balance', () => {
    ...
    const candidates = dedupeCandidates(loadAccountMap(ctx.db, TENANT_ID), [
      balance('g1', 148_233),
      balance('a1', 148_233),
    ])
  ...
  it('offers the on-budget current account, which the old filter excluded', () => {
    ...
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'KBC Zichtrekening'), mirror('g1', 'KBC Zichtrekening')])
    asKind('g1', 'cash')
  ...
  it('offers nothing when the names differ and the balances differ', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Argenta Sparen'), mirror('g1', 'KBC Zichtrekening')])
    asKind('g1', 'cash')
  ...
  it('does not treat two empty accounts as agreeing', () => {
    ...
    expect(
      dedupeCandidates(loadAccountMap(ctx.db, TENANT_ID), [balance('g1', 0), balance('a1', 0)]),
    ).toEqual([])
  })
  ...
  it('does not pair a credit card in the red with savings of the same magnitude', () => {
    ...
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Buffer'), mirror('g1', 'Kaart')])
    asKind('g1', 'cash')

    expect(
      dedupeCandidates(loadAccountMap(ctx.db, TENANT_ID), [balance('g1', -80_000), balance('a1', 80_000)]),
    ).toEqual([])
  })
  ...
  it('does not call a currency match on its own evidence', () => {
    ...
    expect(
      dedupeCandidates(loadAccountMap(ctx.db, TENANT_ID), [balance('g1', 12_300), balance('a1', 45_600)]),
    ).toEqual([])
  })
  ...
  it('will not pair a portfolio with a cash account, or the reverse', () => {
    ...
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Bolero'), ghostfolio('g1', 'Bolero')])
    asKind('a1', 'cash')
  ...
  it('matches on containment, whole words only', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Argenta'), mirror('g1', 'Argenta Zichtrekening')])
    asKind('g1', 'cash')
  ...
  it('does not match a word that merely starts another', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Cashflow'), mirror('g1', 'Cash')])
    asKind('g1', 'cash')
  ...
  it('says nothing once the accounts are grouped', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a2', 'Bolero', true), ghostfolio('g1', 'Bolero')])
    const [mirrorRow, truth] = rows()
    setDedupeGroup(ctx.db, TENANT_ID, 'broker', [mirrorRow!.id, truth!.id], truth!.id)

    expect(dedupeCandidates(loadAccountMap(ctx.db, TENANT_ID))).toEqual([])
  })

  it('says nothing about an account excluded from net worth', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Argenta Sparen'), mirror('g1', 'Argenta Sparen')])
    asKind('g1', 'cash')
    updateAccountMap(ctx.db, TENANT_ID, idOf('g1'), { includeInNetWorth: false })
  ...
  it('says nothing about an Actual account excluded from net worth', () => {
    ...
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Argenta Sparen'), mirror('g1', 'Argenta Sparen')])
    asKind('g1', 'cash')
    updateAccountMap(ctx.db, TENANT_ID, idOf('a1'), { includeInNetWorth: false })
  ...
  it('stops offering a dismissed account, even after a sync renames it', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Argenta Sparen'), mirror('g1', 'Argenta Sparen')])
    asKind('g1', 'cash')
    expect(dedupeCandidates(loadAccountMap(ctx.db, TENANT_ID))).toHaveLength(1)

    dismissMirror(ctx.db, TENANT_ID, idOf('g1'))
    expect(dedupeCandidates(loadAccountMap(ctx.db, TENANT_ID))).toEqual([])
    ...
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Argenta Sparen'), mirror('g1', 'Argenta Spaarboekje')])
    expect(dedupeCandidates(loadAccountMap(ctx.db, TENANT_ID))).toEqual([])
  })
})
...
describe('dismissMirror', () => {
  it('decides dedupeGroup without grouping anything', () => {
    syncAccountMap(ctx.db, TENANT_ID, [mirror('g1', 'Argenta Sparen')])
    const [row] = rows()

    const after = dismissMirror(ctx.db, TENANT_ID, row!.id)
  ...
  it('refuses on an account that is in a group, rather than freezing it', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Bolero', true), ghostfolio('g1', 'Bolero')])
    const [a1, g1] = rows()
  ...
  it('is a no-op the second time', () => {
    ...
    dismissMirror(ctx.db, TENANT_ID, row!.id)
    const after = dismissMirror(ctx.db, TENANT_ID, row!.id)
  ...
  it('says nothing happened for an id it does not know', () => {
    expect(dismissMirror(ctx.db, TENANT_ID, 'nope')).toBeNull()
  })
  ...
  it('leaves a derived mirror alone once it has been dismissed', () => {
    ...
    updateAccountMap(ctx.db, TENANT_ID, g1!.id, { kind: 'cash' })
    dismissMirror(ctx.db, TENANT_ID, g1!.id)

    const [pair] = deriveMirrors(loadAccountMap(ctx.db, TENANT_ID))
    expect(pair).toBeDefined()
    expect(applyDerivedMirror(ctx.db, TENANT_ID, pair!)).toBeNull()
    expect(loadAccountMap(ctx.db, TENANT_ID).every((row) => row.dedupeGroup === null)).toBe(true)
  })
})
...
describe('setDedupeGroup', () => {
  it('marks exactly one row as the source of truth', () => {
    ...
    const all = rows()
    setDedupeGroup(ctx.db, TENANT_ID, 'broker', all.map((row) => row.id), all[2]!.id)
  ...
  it('refuses a source of truth that is not in the group', () => {
    ...
    expect(() => setDedupeGroup(ctx.db, TENANT_ID, 'broker', [all[0]!.id], all[1]!.id)).toThrow(
...
describe('decided fields', () => {
  ...
  const seed = (sighting: AccountSighting = actual('a1', 'Current')): string => {
    syncAccountMap(ctx.db, TENANT_ID, [sighting])
    const row = one()
  ...
  it('records the fields a PATCH named', () => {
    const id = seed()
    updateAccountMap(ctx.db, TENANT_ID, id, { kind: 'savings' })
    expect([...decidedFields(one() ?? { decidedFields: null })]).toEqual(['kind'])

    updateAccountMap(ctx.db, TENANT_ID, id, { includeInNetWorth: false })
  ...
  it('records a field set to the value a rule would have chosen anyway', () => {
    ...
    updateAccountMap(ctx.db, TENANT_ID, id, { kind: 'checking' })
    expect([...decidedFields(one() ?? { decidedFields: null })]).toEqual(['kind'])
  })
  ...
  it('leaves provenance alone when the patch is empty, since that is a read', () => {
    const id = seed()
    updateAccountMap(ctx.db, TENANT_ID, id, { kind: 'savings' })
    updateAccountMap(ctx.db, TENANT_ID, id, {})
    expect([...decidedFields(one() ?? { decidedFields: null })]).toEqual(['kind'])
  })

  it('survives a sync, which is the whole point', () => {
    const id = seed()
    updateAccountMap(ctx.db, TENANT_ID, id, { kind: 'savings', includeInNetWorth: false })
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Current account renamed')])
...
describe('grouping records provenance too', () => {
  ...
  it('marks the whole group when one row is named the source of truth', () => {
    ...
    const [a, g] = rows()
    if (a === undefined || g === undefined) throw new Error('expected two rows')
    setDedupeGroup(ctx.db, TENANT_ID, 'grp', [a.id, g.id], g.id)
    ...
    setSourceOfTruth(ctx.db, TENANT_ID, a.id)
...
describe('unlinkGroup', () => {
  ...
  it('separates both members of a pair, whichever id is passed', () => {
    ...
    const after = unlinkGroup(ctx.db, TENANT_ID, a.id)
  ...
  it('gives the same result unlinking from the source-of-truth side', () => {
    ...
    unlinkGroup(ctx.db, TENANT_ID, g.id)
  ...
  it('separates every member of a group larger than two', () => {
    ...
    groupAccounts(ctx.db, TENANT_ID, [a.id, g1.id, g2.id], g1.id)

    const after = unlinkGroup(ctx.db, TENANT_ID, g2.id)
  ...
  it('does nothing to an account that was never grouped', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Mirror')])
    const [a] = rows()
    if (a === undefined) throw new Error('expected a row')

    expect(unlinkGroup(ctx.db, TENANT_ID, a.id)).toEqual([])
    expect(rows()[0]?.dedupeGroup).toBeNull()
  })

  it('does nothing for an unknown id', () => {
    expect(unlinkGroup(ctx.db, TENANT_ID, 'does-not-exist')).toEqual([])
  })
})

describe('applyDerivedFields', () => {
  const seedOne = (): string => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Current')])
    const row = rows()[0]
    if (row === undefined) throw new Error('the sighting produced no row')
    return row.id
  }
  ...
  it('writes a derived value to a field nobody has decided', () => {
    const id = seedOne()
    applyDerivedFields(ctx.db, TENANT_ID, id, { kind: 'savings' })
    expect(rows()[0]?.kind).toBe('savings')
  })

  it('refuses to overwrite a decided field', () => {
    const id = seedOne()
    updateAccountMap(ctx.db, TENANT_ID, id, { kind: 'cash' })
    applyDerivedFields(ctx.db, TENANT_ID, id, { kind: 'savings' })
  ...
  it('does not make a derived value a decision, so a better rule can still improve it', () => {
    ...
    applyDerivedFields(ctx.db, TENANT_ID, id, { kind: 'cash' })
    expect(rows()[0]?.kind).toBe('cash')
  })
  ...
  it('writes the fields it may and skips the ones it may not, in one call', () => {
    ...
    const id = seedOne()
    updateAccountMap(ctx.db, TENANT_ID, id, { includeInNetWorth: false })
    applyDerivedFields(ctx.db, TENANT_ID, id, { kind: 'savings', includeInNetWorth: true })
  ...
  it('stamps the run even when every field it offered was refused', () => {
    ...
    applyDerivedFields(ctx.db, TENANT_ID, id, { kind: 'savings' }, at)
  ...
  it('returns null for an account that does not exist', () => {
    expect(applyDerivedFields(ctx.db, TENANT_ID, crypto.randomUUID(), { kind: 'cash' })).toBeNull()
  })
})
...
describe('deriveMirrors', () => {
  const derive = (): ReturnType<typeof deriveMirrors> => deriveMirrors(loadAccountMap(ctx.db, TENANT_ID))
  ...
  it('pairs a Ghostfolio cash account with the Actual account of the same name', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Argenta zichtrekening'), mirror('g1', 'Argenta zichtrekening')])
  ...
  it('matches through case, diacritics and punctuation', () => {
    ...
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Argenta — Zichtrekening'), mirror('g1', 'argenta zichtrekéning')])
  ...
  it('leaves a Ghostfolio account that holds positions alone', () => {
    ...
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Bolero'), ghostfolio('g1', 'Bolero', true)])
  ...
  it('refuses a name two Actual accounts share', () => {
    ...
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Spaarrekening'), actual('a2', 'spaarrekening'), mirror('g1', 'Spaarrekening')])

    expect(derive()).toEqual([])
  })

  it('refuses a name two Ghostfolio accounts share', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Spaarrekening'), mirror('g1', 'Spaarrekening'), mirror('g2', 'spaarrekening ')])
  ...
  it('says nothing about a Ghostfolio account with no twin', () => {
    ...
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Current'), mirror('g1', 'Revolut')])
  ...
  it('ignores a name that normalises to nothing', () => {
    ...
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', '—'), mirror('g1', '-')])
  ...
  it('skips rows that are already grouped, however they got that way', () => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Argenta'), mirror('g1', 'Argenta')])
    setDedupeGroup(ctx.db, TENANT_ID, 'grp', [idOf('a1')], idOf('a1'))
...
describe('applyDerivedMirror', () => {
  const seedPair = (): { actualId: string; ghostfolioId: string } => {
    syncAccountMap(ctx.db, TENANT_ID, [actual('a1', 'Argenta'), mirror('g1', 'Argenta')])
    const [mirrorPair] = deriveMirrors(loadAccountMap(ctx.db, TENANT_ID))
  ...
  const byId = (id: string) => {
    const row = loadAccountMap(ctx.db, TENANT_ID).find((candidate) => candidate.id === id)
  ...
  it('groups the pair and makes Actual the side that counts', () => {
    ...
    const group = applyDerivedMirror(ctx.db, TENANT_ID, { actualId, ghostfolioId, matchedOn: 'argenta' })
  ...
  it('refuses rather than half-grouping when one row has gone', () => {
    ...
    const missing = applyDerivedMirror(ctx.db, TENANT_ID, {
      actualId,
  ...
  it('is a no-op the second time, because the pair is now grouped', () => {
    const { actualId, ghostfolioId } = seedPair()
    const first = applyDerivedMirror(ctx.db, TENANT_ID, { actualId, ghostfolioId, matchedOn: 'argenta' })
    const second = applyDerivedMirror(ctx.db, TENANT_ID, { actualId, ghostfolioId, matchedOn: 'argenta' })
    ...
    expect(deriveMirrors(loadAccountMap(ctx.db, TENANT_ID))).toEqual([])
  })
})
