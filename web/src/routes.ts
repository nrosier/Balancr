...
export interface AppRoute {
   ...
   * does its own sub-navigation (Settings, #200) — every other route still 404s on a
   * nested path (#30 onward), which `nav.tsx`'s `isActive` highlights but this does
   * not resolve to a page.
   */
...
export const ROUTES: readonly AppRoute[] = [
  ...
  { path: '/insights', labelKey: 'nav.insights', Icon: IconInsights, Page: Insights },
