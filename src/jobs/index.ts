 ...
 * `account_map` rows and the facts that `networth` and the signals read. Every job
 * shares one queue, so listing them in this order is all the sequencing there is.
 */
...
export const registry: readonly Job[] = [syncJob, portfolioJob, netWorthJob]
